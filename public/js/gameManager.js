...
class GameManager {
        ...
        this.pokemonData.set('python', {
            ...
            abilities: [
                ...
                },
                {
                    ...
                    power: 20,
                    accuracy: 85,
                    description: 'Prevents target from using multi-core abilities for 2 turns.',
                    cooldown: 3
                }
            ]
        });
        ...
        this.pokemonData.set('go', {
            ...
            abilities: [
                ...
                },
                {
                    ...
                    power: 15,
                    accuracy: 100,
                    description: 'Cleanses status effects. Gains shield vs type coercion for 1 turn.',
                    cooldown: 3
                }
            ]
        });
        ...
        this.pokemonData.set('rust', {
            ...
            abilities: [
                ...
                },
                {
                    ...
                    power: 20,
                    accuracy: 80,
                    description: 'Removes one random buff or inflicts Rust-Bound for 1 turn.',
                    cooldown: 3
                }
            ]
        });
        ...
        this.pokemonData.set('cpp', {
            ...
            abilities: [
                ...
                },
                {
                    ...
                    power: 15,
                    accuracy: 100,
                    description: 'Grants Compatibility Shield for 2 turns vs Script/Web damage.',
                    cooldown: 4
                }
            ]
        });
    ...
    getEffectiveStat(pokemon, stat) {
        ...
        console.log(`${pokemon.name} ${stat}: Base=${baseStat}, Buffs=+${buffStages}, Debuffs=-${debuffStages}, Net=${netStages}, Multiplier=${multiplier.toFixed(2)}, Effective=${effectiveStat}`);
    ...
    applyBuff(pokemon, stat, stages = 1) {
        ...
        console.log(`BUFF APPLIED: ${pokemon.name} ${stat} +${stages} stages (${oldBuffs} → ${pokemon.buffs[stat]})`);
    ...
    applyDebuff(pokemon, stat, stages = 1) {
        ...
        console.log(`DEBUFF APPLIED: ${pokemon.name} ${stat} -${stages} stages (${oldDebuffs} → ${pokemon.debuffs[stat]})`);
        console.log(`   ${pokemon.name} ${stat}: ${oldValue} → ${newValue}`);
    ...
    calculateDamage(attacker, defender, ability) {
        ...
        console.log(`DAMAGE CALCULATION: ${attacker.name} uses ${ability.name} (${ability.type}, ${basePower} BP) vs ${defender.name}`);
        ...
        if (ability.type === 'Physical') {
            ...
            console.log(`   Using Physical stats: Attack=${attackStat}, Defense=${defenseStat}`);
        } else {
            attackStat = this.getEffectiveStat(attacker, 'specialAttack');
            defenseStat = this.getEffectiveStat(defender, 'specialDefense');
            console.log(`   Using Special stats: SpAtk=${attackStat}, SpDef=${defenseStat}`);
        }
        
        // Damage formula: (Base Power * Attack / Defense) * Random Factor
        const randomFactor = Math.random() * 0.3 + 0.85;
        const damage = Math.floor((basePower * attackStat / defenseStat) * randomFactor);
        const finalDamage = Math.max(1, damage); // Minimum 1 damage
        
        console.log(`   Formula: (${basePower} × ${attackStat} ÷ ${defenseStat}) × ${randomFactor.toFixed(2)} = ${finalDamage} damage`);
        console.log(`   ${defender.name} HP: ${defender.hp} → ${defender.hp - finalDamage}`);
