...
class BattleScene extends Phaser.Scene {
	...
	create() {
		...
		this.turnText = this.add
			.text(480, 540, `Turn: ${this.pokemon1.name}`, {
				fontSize: "16px",
				fill: "#fff",
				stroke: "#000",
				strokeThickness: 2,
	...
	createAbilityUI() {
		...
		abilities.forEach((ability, index) => {
			...
			button.on("pointerdown", () => {
				console.log("Ability clicked time to play sound: ", ability);
				this.playAbilitySound(ability.type);
	...
	executePlayerAttack() {
		...
		if (accuracy <= ability.accuracy) {
			...
			this.battleText.setText(
				`${attacker.name} used ${ability.name}! It dealt ${damage} damage!`
			);
